...
namespace ABTTestProgram {
    internal sealed partial class ABTTests {
        ...
        internal static String P00300(Test test, Dictionary<String, Instrument> instruments) {
            ...
            _dialogResult = MessageBox.Show($"Connect PICkit 4 IC programmer to ABTTestProgram UUT per Table 7.{Environment.NewLine}{Environment.NewLine}" +
            ...
            E3610xB.Off(instruments[Instrument.POWER_SECONDARY]);
            _dialogResult = MessageBox.Show($"Disconnect PICkit 4 IC programmer from ABTTestProgram UUT.{Environment.NewLine}{Environment.NewLine}" +
        ...
        internal static String P00400(Test test, Dictionary<String, Instrument> instruments) {
            InstrumentTasks.Reset(instruments);
            _dialogResult = MessageBox.Show($"Connect PICkit 4 IC programmer to ABTTestProgram UUT per Table 9.{Environment.NewLine}{Environment.NewLine}" +
                $"Press OK when completed, Cancel to cancel.", "Connect PICkit 4", MessageBoxButtons.OKCancel);
